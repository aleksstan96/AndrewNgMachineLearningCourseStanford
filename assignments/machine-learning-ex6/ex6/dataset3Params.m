...
prediction_error = zeros(1,length(C_set)*length(sigma_set));
index = 0;
...
for i=1:length(C_set)
	for j=1:length(sigma_set)
		...
		predictions =  svmPredict(model, Xval);
		prediction_error(1,index) = mean(double(predictions ~= yval));
	end;
end;


[min, pos] = min(prediction_error);

C = C_set(ceil(pos/8));
sigma = sigma_set(mod(pos,8));
